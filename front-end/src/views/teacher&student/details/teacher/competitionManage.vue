<template>
  <!--竞赛成绩管理-->
    <div style="top:100px;position: absolute" class="competitionManage">
        ...
        <div>
            <div style="float: left;margin-top: 5px;color:#606266">竞赛id: {{cp.competitionId}}&nbsp;&nbsp;&nbsp;&nbsp; 竞赛名称: {{cp.competitionName}}&nbsp;&nbsp;&nbsp;&nbsp;竞赛分类: {{cp.subject}}
                &nbsp;&nbsp;&nbsp; 竞赛人数:{{cp.count}}
                <br>竞赛时间: {{time.formatDate(cp.competitionTime)}}&nbsp;&nbsp;&nbsp;&nbsp
                <el-button @click="submit(cp.competitionId)">发布成绩</el-button>
        ...
        <el-table  :data="tableData" stripe style="width: 100%;height: 500px" >
            ...
            <el-table-column prop="grade" label="成绩" width="100">
                <template  #default="scope">
                    <div>
                        <el-input v-model="tableData[scope.$index].grade">{{tableData[scope.$index].grade}}</el-input>
                    </div>
                </template>
            </el-table-column>
            <el-table-column prop="zpUrl" label="作品下载" width="400">
                <template  #default="scope">
                    <div>
...
<!--        </el-dialog>-->
        ...
        <div style="display: flex; justify-content: center; align-items: center;height: 80px;background-color: white">
            <el-pagination
...
const submit=(cid)=>{
    ...
    if (tableData.value==null)
        return ElMessage.error("没有竞赛成绩可以发布")
    axios.post("/personal/submitGrade?cid="+cid,tableData.value,{headers:axios2.accessHeader()})
        .then((res)=>{
            console.log(res)
            if (res.data.code===400)
                return ElMessage.warning(res.data.message)
               ElMessage.success("竞赛成绩发布成功!")
        })
}
//获取竞赛信息
const getCompInfo=()=>{
    axios2.getUserInfo((res)=>{
        uid.value=res.data.uid
        axios.get("/personal/selectGradeComp?currentPage="+currentPage.value+"&uid="+uid.value,{headers:axios2.accessHeader()})
            .then((res)=>{
                if (res.data.code===200)
                {
                    ...
                    cp.count=res.data.data.list[0].count
                    totalCount.value=res.data.data.totalCount
                    size.value=res.data.data.size
                    //获取对应竞赛的学生，给学生成绩
                    axios.get("/personal/selectGrade?cid="+cp.competitionId,{headers:axios2.accessHeader()})
                        .then((res)=>{
                            console.log(res)
                            if (res.data.code===200)
